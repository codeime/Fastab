...
- 修复：远程 IPC 重连时，若仍有在途 outbox 帧占用配额，不再被当成永久停止（区分 `Busy` 与 `Stopped`）
- 修复：拦截键回放不再把 PTY 主循环卡住最多约 5 秒；普通输入与桌面 Insert 仍排在未结算拦截键之后，但不再同步 barrier 等待
- 修复：generator 子进程在 kill 后的清理保持有界；未及时退出的子进程改为后台 `waitpid`，避免无限 `wait` 或僵尸进程
- 修复：启用 Jev AI 推荐时，若 bundled specs-ir 的公开基线 pin 不匹配，设置页会明确失败提示而不是静默无效
- 修复：工作区通过 CI 的 `cargo fmt --check`
- 变更：终端 grid 行缓存回收带滞回，短暂改尺寸时不再反复伸缩容量
- 新功能：可选的 Jev 补全推荐（设置与供应商配置）；本地候选就绪后可将其中一个提到列表首位
- 修复：Jev API Key 直接处理 ASCII 按键，绕开 macOS 文本组合；粘贴时接受首尾空白，空白或非法粘贴保留原内容
- 修复：未改变内容的设置输入不再暂停 Jev；凭据或配置保存失败时保留密钥草稿，可直接重试
- 变更：Jev 设置单独成为一个设置页
