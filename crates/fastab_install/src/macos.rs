...
use fastab_util::consts::{APP_BUNDLE_ID, APP_PROCESS_NAME, CLI_BINARY_NAME, system_paths};
...
async fn unregister_fastab_login_item() {
    if let Some(desktop) = login_item_unregister_binary(&fastab_util::app_bundle_path()) {
    ...
    if let Err(err) = fastab_integrations::login_item::set_enabled(false) {
