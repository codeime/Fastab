...
use fastab_util::directories::{figterm_socket_path, logs_dir, update_lock_path};
        ...
        match self {
            ...
            InternalSubcommand::SshLocalCommand { remote_dest, uuid } => {
                ...
                if let Ok(session_id) = std::env::var(QTERM_SESSION_ID) {
                    let mut conn =
                        BufferedUnixStream::connect(fastab_util::directories::figterm_socket_path(&session_id)?)
                            .await?;
...
pub async fn pre_cmd(alias: Option<String>) -> ExitCode {
    ...
    match figterm_socket_path(&session_id) {
        Ok(figterm_path) => match fastab_ipc::socket_connect(figterm_path).await {
