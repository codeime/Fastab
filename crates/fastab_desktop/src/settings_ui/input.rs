...
use gpui::prelude::*;
use gpui::{App, Bounds, ClipboardItem, Context, ElementInputHandler, EntityInputHandler,
    FocusHandle, Focusable, KeyDownEvent, MouseButton, Pixels, Point, ShapedLine,
    SharedString, TextRun, UTF16Selection, Window, canvas, div, fill, point, px, rgb, size};
...
impl Input {
    pub(super) fn new(value: String, password: bool, limit: usize, cx: &mut Context<Self>) -> Self {
        Self { value: Self::bounded(value, limit), password, limit, focus: cx.focus_handle(), anchor: 0, cursor: 0,
            marked: None, layout: None, origin: point(px(0.), px(0.)), scroll: px(0.),
            dragging: false, edit_revision: 0, enabled: true }
    }

    pub(super) fn value(&self) -> &str { &self.value }

    pub(super) fn edit_revision(&self) -> u64 { self.edit_revision }
    ...
    fn selection(&self) -> Range<usize> { self.anchor.min(self.cursor)..self.anchor.max(self.cursor) }

    fn utf8(text: &str, utf16: usize) -> usize {
        let mut offset = 0;
        for (byte, ch) in text.char_indices() {
            if offset >= utf16 { return byte; }
            offset += ch.len_utf16();
        }
        text.len()
    }

    fn utf16(&self, byte: usize) -> usize { self.value[..byte].encode_utf16().count() }
    ...
    fn display_index(&self, byte: usize) -> usize {
        if self.password { self.value[..byte].chars().count() } else { byte }
    }

    fn actual_index(&self, display: usize) -> usize {
        if self.password {
            self.value.char_indices().nth(display).map_or(self.value.len(), |(byte, _)| byte)
        } else {
            let mut byte = display.min(self.value.len());
            while !self.value.is_char_boundary(byte) { byte -= 1; }
            byte
        }
    }
    ...
    fn replace(&mut self, range: Range<usize>, text: &str, cx: &mut Context<Self>) -> bool {
        if !self.enabled || text.chars().any(char::is_control)
    ...
    fn key(&mut self, event: &KeyDownEvent, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if modifiers.platform {
            match key {
                "a" => { self.anchor = 0; self.cursor = self.value.len(); },
                "c" | "x" => {
                    if !self.password && !self.selection().is_empty() {
                        ...
                        if key == "x" { self.replace(self.selection(), "", cx); }
                    }
                },
                "v" => {
                ...
                "left" => { self.cursor = 0; if !modifiers.shift { self.anchor = 0; } },
                "right" => { self.cursor = self.value.len(); if !modifiers.shift { self.anchor = self.cursor; } },
                _ => handled = false,
            }
            ...
            match key {
                "backspace" | "delete" => {
                    ...
                    if range.is_empty() {
                        if key == "backspace" {
                            range.start = self.value[..self.cursor].char_indices().next_back().map_or(0, |(byte, _)| byte);
                        } else {
                            range.end = self.value[self.cursor..].chars().next().map_or(self.cursor, |ch| self.cursor + ch.len_utf8());
                        }
                    }
                ...
                "left" | "right" | "home" | "end" => {
                    self.cursor = match key {
                        ...
                        "left" => self.value[..self.cursor].char_indices().next_back().map_or(0, |(byte, _)| byte),
                        _ => self.value[self.cursor..].chars().next().map_or(self.cursor, |ch| self.cursor + ch.len_utf8()),
                    };
                    if !modifiers.shift { self.anchor = self.cursor; }
                    self.marked = None;
                },
                "enter" => {},
                _ => handled = false,
        ...
        if handled { cx.stop_propagation(); cx.notify(); }
    }
}
...
impl EntityInputHandler for Input {
    fn text_for_range(&mut self, range: Range<usize>, adjusted: &mut Option<Range<usize>>, _: &mut Window, _: &mut Context<Self>) -> Option<String> {
        if self.password { *adjusted = None; return None; }
    ...
    fn selected_text_range(&mut self, _: bool, _: &mut Window, _: &mut Context<Self>) -> Option<UTF16Selection> {
        ...
        Some(UTF16Selection { range: self.utf16(range.start)..self.utf16(range.end), reversed: self.cursor < self.anchor })
    }
    fn marked_text_range(&self, _: &mut Window, _: &mut Context<Self>) -> Option<Range<usize>> {
        self.marked.as_ref().map(|range| self.utf16(range.start)..self.utf16(range.end))
    }
    fn unmark_text(&mut self, _: &mut Window, cx: &mut Context<Self>) { self.marked = None; cx.notify(); }
    fn replace_text_in_range(&mut self, range: Option<Range<usize>>, text: &str, _: &mut Window, cx: &mut Context<Self>) {
        let range = range.map(|range| self.range(range)).or(self.marked.clone()).unwrap_or_else(|| self.selection());
        self.replace(range, text, cx);
    }
    fn replace_and_mark_text_in_range(&mut self, range: Option<Range<usize>>, text: &str, selected: Option<Range<usize>>, _: &mut Window, cx: &mut Context<Self>) {
        let range = range.map(|range| self.range(range)).or(self.marked.clone()).unwrap_or_else(|| self.selection());
    ...
    fn bounds_for_range(&mut self, range: Range<usize>, bounds: Bounds<Pixels>, _: &mut Window, _: &mut Context<Self>) -> Option<Bounds<Pixels>> {
        ...
        let range = self.range(range);
        Some(Bounds::from_corners(point(self.origin.x + line.x_for_index(self.display_index(range.start)), bounds.top()),
            point(self.origin.x + line.x_for_index(self.display_index(range.end)), bounds.bottom())))
    }
...
impl Render for Input {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div().id("jev-input").w_full().min_w(px(0.)).h(px(34.)).px(px(8.)).py(px(6.))
            .border_1().border_color(rgb(chrome.separator)).rounded_md().bg(rgb(chrome.bg))
            .text_size(px(13.)).text_color(rgb(chrome.text)).overflow_hidden()
            .track_focus(&self.focus).tab_stop(true).cursor(gpui::CursorStyle::IBeam)
            ...
            .on_mouse_down(MouseButton::Left, cx.listener(|this, event: &gpui::MouseDownEvent, window, cx| {
                ...
                if !event.modifiers.shift { this.anchor = this.cursor; }
                this.dragging = true;
                cx.stop_propagation(); cx.notify();
            }))
            .on_mouse_move(cx.listener(|this, event: &gpui::MouseMoveEvent, _, cx| {
                if this.dragging { this.cursor = this.mouse_index(event.position); cx.notify(); }
            }))
            ...
            .child(canvas(|_, _, _| (), move |bounds, (), window, cx| {
                entity.update(cx, |input, cx| {
                    let style = window.text_style();
                    let display: SharedString = if input.password { "*".repeat(input.value.chars().count()).into() } else { input.value.clone().into() };
                    let run = TextRun { len: display.len(), font: style.font(), color: style.color,
                    ...
                    if input.focus.is_focused(window) {
                        if !selection.is_empty() {
                            window.paint_quad(fill(Bounds::from_corners(
                                point(origin.x + line.x_for_index(input.display_index(selection.start)), bounds.top()),
                                point(origin.x + line.x_for_index(input.display_index(selection.end)), bounds.bottom())), rgb(chrome.selection)));
                        } else {
                            window.paint_quad(fill(Bounds::new(point(origin.x + cursor, bounds.top()), size(px(1.), bounds.size.height)), rgb(chrome.accent)));
                        }
                    }
