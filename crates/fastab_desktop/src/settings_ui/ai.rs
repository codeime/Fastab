...
impl AiSettings {
    pub(super) fn new(proxy: EventLoopProxy, cx: &mut Context<Self>) -> Self {
        ...
        let acknowledged_service = (profile.data_policy_version == DATA_POLICY_VERSION)
            .then(|| profile.credential_key().ok()).flatten();
        ...
        let input_observations = [
            cx.observe(&key, |this, input, cx| { let revision = input.read(cx).edit_revision(); this.input_edited(0, revision, cx); }),
            cx.observe(&model, |this, input, cx| { let revision = input.read(cx).edit_revision(); this.input_edited(1, revision, cx); }),
            cx.observe(&base, |this, input, cx| { let revision = input.read(cx).edit_revision(); this.input_edited(2, revision, cx); }),
        ];
        Self {
            enabled: config.enabled, config, profile: profile.clone(),
            key, model, base,
            acknowledged_service, advanced: false, busy: false, load_failed, persistence_failed: false, epoch: 0,
            status: if load_failed { Self::label("无法读取 AI 配置；请关闭并重新打开设置。", "Could not read AI settings. Close and reopen settings.").into() } else { String::new() },
            buttons: BTreeMap::new(), proxy, edit_revisions: [0; 3], _input_observations: input_observations,
        }
    }
    ...
    pub(super) fn clear_draft(&mut self, cx: &mut Context<Self>) {
        ...
        self.key.update(cx, |input, cx| { input.take(cx); });
        cx.notify();
    }
    ...
    fn draft(&self, cx: &App) -> Profile {
        ...
        profile.data_policy_version = if profile.credential_key().ok().as_ref() == self.acknowledged_service.as_ref()
            && self.acknowledged_service.is_some() { DATA_POLICY_VERSION } else { 0 };
        profile
    }
    ...
    fn select(&mut self, profile: Profile, cx: &mut Context<Self>) {
        ...
        self.base.update(cx, |input, cx| input.set(profile.base_url.clone(), cx));
        self.profile = profile;
        cx.notify();
    }

    fn provider(&mut self, provider: Provider, cx: &mut Context<Self>) {
        let profile = self.config.profiles.iter().find(|profile| profile.provider == provider).cloned()
            .unwrap_or_else(|| match provider {
                Provider::TypeSafe => Profile::typesafe(),
                Provider::OpenRouter => Profile::openrouter(),
                Provider::CustomSystemOne => Profile { id: uuid::Uuid::new_v4().to_string(), provider,
                    base_url: String::new(), model: "jev-1.13.0".into(), data_policy_version: 0 },
            });
    ...
    fn set_busy(&mut self, busy: bool, cx: &mut Context<Self>) {
        ...
        for field in [&self.key, &self.model, &self.base] {
            field.update(cx, |input, cx| { input.enabled = !busy; cx.notify(); });
        }
        cx.notify();
    }

    fn changed(&self) { let _ = self.proxy.send_event(Event::JevSettingsChanged); }
    ...
    fn observe_current_edits(&mut self, cx: &mut Context<Self>) {
        let revisions = [self.key.read(cx).edit_revision(), self.model.read(cx).edit_revision(), self.base.read(cx).edit_revision()];
    ...
    fn suspend_for_edit(&mut self, cx: &mut Context<Self>) {
        ...
        self.epoch = self.epoch.wrapping_add(1);
        let saved = self.persist(&disabled, Self::label("暂停旧配置时保存失败。", "Saving the paused configuration failed."), cx);
        self.config = disabled;
        if saved {
            self.changed();
            self.status = Self::label("旧配置已暂停；完成编辑并明确保存后才重新启用。", "The previous configuration is paused. Edit and explicitly save to enable again.").into();
        }
        cx.notify();
    }
    ...
    fn persist(&mut self, config: &AiConfig, failure: &str, cx: &mut Context<Self>) -> bool {
        if config.save().is_err() {
            ...
            self.changed();
            self.status = format!("{failure} {}", Self::label(
    ...
    fn credential_error(error: CredentialError) -> &'static str {
        match error {
            CredentialError::Busy => Self::label("凭据操作正在完成，请稍后重试；AI 保持关闭。", "A credential operation is still finishing. Retry shortly; AI remains off."),
            CredentialError::Unavailable | CredentialError::InvalidService => Self::label("当前凭据不可用；AI 保持关闭。可重新保存密钥。", "Credentials are currently unavailable; AI remains off. You can save a new key."),
        }
    }

    fn save(&mut self, cx: &mut Context<Self>) {
        if self.busy || self.load_failed { return; }
        self.observe_current_edits(cx);
        self.suspend_for_edit(cx);
        ...
        let service = match profile.credential_key() {
            Ok(service) => service,
            Err(_invalid_address) => { self.status = Self::label("Base URL 无效：预设地址固定，自定义地址须为 HTTPS 基地址。", "Invalid Base URL: presets are fixed; custom profiles require an HTTPS base address.").into(); cx.notify(); return; },
        };
        if desired_enabled && profile.validate().is_err() {
            self.status = Self::label("启用前请确认数据范围，并使用已支持的 Jev 模型。", "Confirm the data scope and select a supported Jev model before enabling.").into();
            cx.notify(); return;
        }
        ...
        profile.id = self.config.profiles.iter()
            .find(|old| old.credential_key().ok().as_deref() == Some(service.as_str()))
            .map(|old| old.id.clone()).unwrap_or_else(|| uuid::Uuid::new_v4().to_string());
        let mut disabled = self.config.clone();
        if disabled.upsert_profile(profile.clone()).is_err() {
            self.status = Self::label("配置未保存：请检查模型、地址及配置数量（最多 16 个）。", "Settings were not saved. Check the model, address and profile limit (16).").into(); cx.notify(); return;
        }
        disabled.active_profile_id = Some(profile.id.clone());
        disabled.enabled = false;
        if !self.persist(&disabled, Self::label("无法保存配置；未修改密钥。", "Could not save settings; the key was not changed."), cx) { return; }
        ...
        cx.spawn(async move |this, cx| {
            ...
            let _ = this.update(cx, |this, cx| {
                ...
                if let Err(error) = result { this.status = Self::credential_error(error).into(); return; }
                if runtime_revision() != saved_revision || AiConfig::load().ok().as_ref() != Some(&disabled) {
                    this.status = Self::label("配置已变化；密钥操作已完成，未启用 AI。请重新打开设置。", "Settings changed. The credential operation finished; AI was not enabled. Reopen settings.").into();
                    this.load_failed = true; return;
                }
                let mut final_config = disabled;
                final_config.enabled = desired_enabled;
                if !this.persist(&final_config, Self::label("凭据操作已完成，但配置保存失败。", "The credential operation finished, but saving settings failed."), cx) { return; }
                ...
                } else { Self::label("密钥已保存，AI 保持关闭。", "Key saved. AI remains off.") }.into();
            });
        }).detach();
    }
    ...
    fn delete_profile(&mut self, profile: Profile, cx: &mut Context<Self>) {
        if self.busy || self.load_failed { return; }
        let Ok(service) = profile.credential_key() else { return; };
        ...
        let mut disabled = self.config.clone();
        disabled.enabled = false;
        if !self.persist(&disabled, Self::label("无法保存关闭状态；未删除密钥。", "Could not persist the disabled state; the key was not deleted."), cx) { return; }
        self.config = disabled.clone(); self.enabled = false;
        self.clear_draft(cx); self.changed();
        let epoch = self.epoch;
        let saved_revision = runtime_revision();
        ...
        cx.spawn(async move |this, cx| {
            ...
            let _ = this.update(cx, |this, cx| {
                ...
                if let Err(error) = result { this.status = Self::credential_error(error).into(); return; }
                if runtime_revision() != saved_revision || AiConfig::load().ok().as_ref() != Some(&disabled) {
                    this.status = Self::label("密钥已删除，但配置已变化；请重新打开设置。", "The key was deleted, but settings changed. Reopen settings.").into();
                    this.load_failed = true; return;
                }
                let mut updated = disabled;
                updated.remove_profile(&profile.id);
                if !this.persist(&updated, Self::label("密钥已删除；配置删除失败。", "Key deleted. Removing the profile failed."), cx) { return; }
                this.config = updated;
                if this.profile.id == profile.id {
                    let next = this.config.active_profile().cloned().or_else(|| this.config.profiles.first().cloned()).unwrap_or_else(Profile::typesafe);
                    this.select(next, cx);
                }
                this.changed();
                this.status = Self::label("已删除密钥及配置，AI 已关闭。", "Key and profile deleted. AI is off.").into();
            });
        }).detach();
    }

    fn button(&mut self, id: String, label: String, cx: &mut Context<Self>, action: impl Fn(&mut Self, &mut Context<Self>) + 'static) -> gpui::AnyElement {
        let chrome = Chrome::current();
        let focus = self.buttons.entry(id.clone()).or_insert_with(|| cx.focus_handle()).clone();
        let action = Rc::new(action);
        let click_action = action.clone();
        let click_entity = cx.entity();
        let key_entity = cx.entity();
        let click_focus = focus.clone();
        div().id(gpui::SharedString::from(id)).track_focus(&focus).tab_stop(true)
            .px(px(10.)).py(px(6.)).rounded_md().border_1().border_color(rgb(chrome.separator))
            .text_size(px(12.)).text_color(rgb(chrome.text)).cursor_pointer()
...
impl Render for AiSettings {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        self.buttons.retain(|id, _| {
            let profile_id = id.strip_prefix("jev-select-").or_else(|| id.strip_prefix("jev-delete-"));
        ...
        let destination = normalize_base_url(&draft.base_url).map(|(_, endpoint)| endpoint.to_string())
            .unwrap_or_else(|_| Self::label("尚无有效 HTTPS 地址", "No valid HTTPS destination").into());
        let mut providers = div().flex().flex_wrap().gap(px(6.));
        for (id, label, provider) in [("typesafe", "TypeSafe", Provider::TypeSafe), ("openrouter", "OpenRouter", Provider::OpenRouter), ("custom", "Custom System One", Provider::CustomSystemOne)] {
            let label = if self.profile.provider == provider { format!("● {label}") } else { label.into() };
            providers = providers.child(self.button(format!("jev-{id}"), label, cx, move |this, cx| this.provider(provider, cx)));
        }
        let enable_label = if self.enabled { Self::label("自动推荐：开启", "Automatic recommendations: on") } else { Self::label("自动推荐：关闭", "Automatic recommendations: off") };
        let enable_label = if self.enabled != self.config.enabled {
            format!("{enable_label}{}", Self::label("（待保存）", " (save to apply)"))
        } else { enable_label.into() };
        ...
        let consent_label = if confirmed { Self::label("☑ 已确认上述数据范围与处理方", "☑ Data scope and processors confirmed") } else { Self::label("☐ 确认上述数据范围与处理方", "☐ Confirm data scope and processors") };
        let consent = self.button("jev-consent".into(), consent_label.into(), cx, |this, cx| {
            if this.busy { return; }
            this.suspend_for_edit(cx);
            let service = this.draft(cx).credential_key().ok();
            this.acknowledged_service = if service == this.acknowledged_service { None } else { service };
            cx.notify();
        });
        let advanced = self.button("jev-advanced".into(), Self::label("高级：Base URL", "Advanced: Base URL").into(), cx, |this, cx| { this.advanced = !this.advanced; cx.notify(); });
        let save = self.button("jev-save".into(), Self::label("保存配置 / 替换密钥", "Save settings / replace key").into(), cx, |this, cx| this.save(cx));
        let mut policies = div().flex().flex_wrap().gap(px(6.));
        if self.profile.provider != Provider::CustomSystemOne {
            policies = policies
                .child(self.button("jev-typesafe-privacy".into(), Self::label("TypeSafe 隐私政策", "TypeSafe Privacy Policy").into(), cx, |_, cx| cx.open_url("https://typesafe.ai/legal/privacy-policy")))
                .child(self.button("jev-typesafe-terms".into(), Self::label("TypeSafe 服务条款", "TypeSafe Terms").into(), cx, |_, cx| cx.open_url("https://typesafe.ai/legal/mca")));
        }
        if self.profile.provider == Provider::OpenRouter {
            policies = policies.child(self.button("jev-openrouter-privacy".into(), Self::label("OpenRouter 数据政策", "OpenRouter Data Policy").into(), cx, |_, cx| cx.open_url("https://openrouter.ai/docs/guides/privacy/data-collection")));
        }
        ...
        for profile in self.config.profiles.clone() {
            ...
            let provider = match profile.provider {
                Provider::TypeSafe => "TypeSafe", Provider::OpenRouter => "OpenRouter",
                Provider::CustomSystemOne => "Custom System One",
            };
            let label = format!("{provider} · {}", profile.base_url);
            let select = self.button(format!("jev-select-{}", profile.id), label, cx, move |this, cx| this.select(select_profile.clone(), cx));
            let delete = self.button(format!("jev-delete-{}", profile.id), Self::label("删除密钥及配置", "Delete key and profile").into(), cx, move |this, cx| this.delete_profile(profile.clone(), cx));
            saved = saved.child(div().flex().flex_wrap().gap(px(6.)).child(select).child(delete));
        }
        let processors = match self.profile.provider {
            Provider::TypeSafe => Self::label("处理方：TypeSafe（美国处理）；不承诺零保留。", "Processor: TypeSafe (US processing); no zero-retention promise."),
            Provider::OpenRouter => Self::label("处理方：OpenRouter 及 TypeSafe；数据经过两方，不承诺零保留。", "Processors: OpenRouter and TypeSafe; data passes through both, with no zero-retention promise."),
            Provider::CustomSystemOne => Self::label("处理方：下方自定义地址的运营方及其上游；请自行确认其数据政策。", "Processors: the custom destination below and its upstream providers. Review their data policies."),
        };
        let body = div().p(px(16.)).flex().flex_col().gap(px(12.)).text_size(px(13.))
            ...
            .child(saved);
        super::card(Self::label("AI 候选推荐 · Jev", "AI recommendations · Jev"), chrome, body)
    }
}
