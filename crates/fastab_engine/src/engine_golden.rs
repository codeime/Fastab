...
//! with `EC_ENGINE_GOLDEN_UPDATE=1 cargo test -p fastab_engine engine_golden`.
//!
...
fn install_settings(case: &EngineGoldenCase) -> fastab_settings::settings::SettingsOverrideGuard {
    ...
    fastab_settings::settings::install_override(fastab_settings::settings::Settings::from_slice(&refs))
}
