...
/// Easy Complete's data directory. Fastab is a sibling product and never
/// reads, writes, or migrates this tree.
...
pub fn resources_path_ctx<Ctx: EnvProvider + PlatformProvider>(ctx: &Ctx) -> Result<PathBuf> {
    ...
    match os {
        ...
        fastab_os_shim::Os::Windows => Ok(fig_data_dir()?.join("resources")),
...
mod linux_tests {
    ...
    fn all_paths() {
        ...
        assert_ne!(fig_data_dir().unwrap(), previous_product_data_dir().unwrap());
